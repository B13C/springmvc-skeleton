package com.btn.servlet;

import com.btn.annotations.*;
import com.btn.commons.CommonUtils;
...
public class BtDispatcherServlet extends HttpServlet {
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        for (int i = 0; i < methodParameters.length; i++) {
            if (ServletRequest.class.isAssignableFrom(methodParameters[i].getType())) {
                ...
                if (methodParameters[i].isAnnotationPresent(BtRequestParam.class)) {
                    bindingValue = methodParameters[i].getAnnotation(BtRequestParam.class).value();
                }
                // 从请求中获取参数的值
    ...
    private void reflectBeansInstance() throws Exception {
        ...
        for (String className : beanNames) {
            ...
            if (aClass.isAnnotationPresent(BtController.class)) {// 操作控制层的实例
            ...
            } else if (aClass.isAnnotationPresent(BtService.class)) {// 操作业务层的实例
